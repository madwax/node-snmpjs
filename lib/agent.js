...
Agent.prototype._transmit_response = function _transmit_response(sock, rsp) {
	...
	if( this._log !== null )
	{
		this._log.trace({ raw: rsp.raw, dst: dst, snmpmsg: rsp }, 'Sending SNMP response message');
	}
...
Agent.prototype._do_getset = function _do_getset(req, rsp) {
	...
	req.pdu.varbinds.forEach(function (vb, i) {
		...
		var prq = new ProviderRequest( req.community, req.pdu.op, vb.oid, node );
...
Agent.prototype._do_getnext_one =
	...
	var prq = new ProviderRequest( req.community, req.pdu.op, oid, node );
	var handler;
